...
func (db *DB) ensureSchema(models []ptypes.ModelEntry) error {
	ddl := []string{
		...
		// Plan layer: plans reify the Phase enum. plan_steps orders one
		...
		// Derivation qualifier: derivation_kinds is the seeded lookup of
		// the vocabulary's seven controlled values. derivation_qualifiers attaches
	...
	// Idempotent additive migration: a pre-plan-layer database gets the
