...
- **`pkg/provo`** — PROV-O / RO-Crate exporter of a Tracker graph using the
  `data-labour-prov` vocabulary (M2 of the paper-integration roadmap). Pure reads:
  `provo.ExportTurtle(w, tr, opts)` emits deterministic Turtle conforming to the
  paper's SHACL shapes; `provo.ExportROCrate(dir, tr, opts)` writes a minimal RO-Crate
