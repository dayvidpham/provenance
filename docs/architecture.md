...
Provenance DBOS adapter
  |  adds versioned operation context and workflow identity
  v
...
The adapter derives workflow and step identity from the pinned contract version,
application version, operation identity, and canonical mutation identity. This
creates two important retry outcomes:
...
encoded as a closed `ApplyFailureKind` plus its actionable details.
...
error channel, but checkpoints Provenance domain failures as typed outcome data.
